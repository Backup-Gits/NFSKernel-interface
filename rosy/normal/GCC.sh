...
tg_post_msg "<b>NFSKernel-normal-(rosy)</b>%0ABuilder Name : <code>${KBUILD_BUILD_USER}</code>%0ABuilder Host : <code>${KBUILD_BUILD_HOST}</code>%0ADevice Defconfig: <code>${DEVICE_DEFCONFIG}</code>%0AGCC Version : <code>${KBUILD_COMPILER_STRING}</code>%0AGCC Rootdir : <code>${GCC_ROOTDIR}</code>%0AKernel Rootdir : <code>${KERNEL_ROOTDIR}</code>"

# Compile
compile(){
tg_post_msg "<b>NFSKernel-normal-(rosy):</b><code>Membangun Kernel sedang di muali, Sabar ya sayank. Gak lama kok..</code>"
cd ${KERNEL_ROOTDIR}
